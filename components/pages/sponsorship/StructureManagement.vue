<template>
    <div>
        ...
        <form @submit.prevent="submitManagementStructure" class="space-y-6">
            ...
            <div class="text-base font-medium text-black p-1">Key Management Personnel</div>
            ...
            <div class="lg:flex max-lg:flex-col items-center gap-4 w-full">
                ...
                <div class="space-y-6 mx-1 lg:w-1/2 max-lg:w-full">
                    ...
                    <div class="inline-flex items-center gap-2 w-full">
                        <div class="lg:text-base max-lg:text-sm w-1/2">Previous relevant positions</div>
                        <CommonInputsVariant class="w-full"
                            type="text"
...
<script setup>
import { managementStructureSchema } from '~/validation/formValidationSchama';
...
const { value: structureManagementDesc, errorMessage: structureManagementDescError } = useField('structureManagementDesc')
const { value: ceoName, errorMessage: ceoNameError } = useField('ceoName')
const { value: ceoYearExperience, errorMessage: ceoYearExperienceError } = useField('ceoYearExperience')
const { value: ceoPreviousPosition, errorMessage: ceoPreviousPositionError } = useField('ceoPreviousPosition')
const { value: ceoEducationDegree, errorMessage: ceoEducationDegreeError } = useField('ceoEducationDegree')
const { value: cfoName, errorMessage: cfoNameError } = useField('cfoName')
const { value: cfoYearExperience, errorMessage: cfoYearExperienceError } = useField('cfoYearExperience')
const { value: cfoPreviousPosition, errorMessage: cfoPreviousPositionError } = useField('cfoPreviousPosition')
const { value: cfoEducationDegree, errorMessage: cfoEducationDegreeError } = useField('cfoEducationDegree')
const { value: ctoName, errorMessage: ctoNameError } = useField('ctoName')
const { value: ctoYearExperience, errorMessage: ctoYearExperienceError } = useField('ctoYearExperience')
const { value: ctoPreviousPosition, errorMessage: ctoPreviousPositionError } = useField('ctoPreviousPosition')
const { value: ctoEducationDegree, errorMessage: ctoEducationDegreeError } = useField('ctoEducationDegree')
...
onMounted(async () => {
    ...
    try {
        ...
        if (profileStore.projectId) {
            ...
            console.log('Sponsorship management structure:', sponsorshipStore.sponsorship);
...
const submitManagementStructure = handleSubmit(async (values) => {
    ...
    try {
        ...
        await sponsorshipStore.saveSponsorshipSection(profileStore.projectId, 'managementStructure', managementStructureData);
