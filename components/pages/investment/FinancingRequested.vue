  ...
  <div>
    <div class="lg:max-w-[920px] m-1">
      <CommonPageHeading
        ...
        description="Please select the financial type"
      />

      <form
        @submit.prevent="submitFinancingType"
        class="space-y-3 my-3 w-3/4 max-lg:w-full"
      >
        <div class="gap-3.5 grid grid-cols-2 max-lg:grid-cols-1">
          <div
            class="flex flex-wrap items-center"
            v-for="data in financeTypeData"
            :key="data.id"
          >
            <label
              class="custom-soft-option flex flex-row items-start gap-3 w-full border-none py-4.5"
            >
              <input
                type="radio"
              ...
              <span class="label-text w-full text-start">
                ...
                <span class="text-base-content/80"
                  >{{ data.description || data.dscription }}
                </span>
        ...
        <div class="space-y-4 max-lg:mb-20">
          <div class="inline-flex items-center justify-between gap-2 w-full">
            <div class="lg:text-base max-lg:text-sm w-3/5 max-lg:min-w-32">
              Currency:
            </div>
            <CommonSelectVariant
              class="w-40"
...
const { value: financingTypes, errorMessage: financingTypesError } =
  useField("financingTypes");
const { value: otherFinancingType, errorMessage: otherFinancingTypeError } =
  useField("otherFinancingType");
const { value: currency, errorMessage: currencyError } = useField("currency");
const { value: requestedAmount, errorMessage: requestedAmountError } =
  useField("requestedAmount");
const { value: proposedTenor, errorMessage: proposedTenorError } =
  useField("proposedTenor");
const { value: gracePeriod, errorMessage: gracePeriodError } =
  useField("gracePeriod");
    ...
    if (projectId.value) {
      ...
      if (stepStore.state?.financingType) {
        setValues({
          ...
          otherFinancingType:
            stepStore.state?.financingType.otherFinancingType || "",
