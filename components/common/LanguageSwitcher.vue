<template>
  <div class="relative">
    <div class="absolute -top-5 right-0 z-50">
      ...
      <div ref="dropdownRef" @blur="onClickOutside" v-show="showDropdown" class="flex flex-col justify-start w-full bg-white shadow-2xl rounded-b text-sm px-2 py-1">
        <button @click.prevent="changeLanguage(locale.code)" v-for="locale in locales" :key="locale.code" class="hover:bg-zinc-100 flex items-center gap-2 py-1.5 px-1 rounded" @click="translatePage('en')">
...
const translatePage = async (lang) => {
  // Holen Sie alle Texte der Seite
